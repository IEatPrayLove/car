...
export default class Index extends Component {
  ...
  componentDidShow() {
    ...
    // dispatch({
    //   type: 'storedMoney/getStoredMerchantIdAction',
    //   payload: {
    //     platformId: PLATFORM_ID
    //   },
    //   callback: ({ ok, data }) => {
    //     if (ok) {
    //       const { merchantNo, id } = data
    //       this.setState({
      ...
      <View className="container">
        ...
        <View className="rechargeMerchant flex-row flex-ac flex-sb">
          ...
          <View 
            className="changeBtn"
            onClick={() => {
              navToPage(`/package/storedMoney/rechargeMerchant/rechargeMerchant?merchantId=${merchant.id}`)
            }}
          >
            切换充值门店
          </View>
