...
//   toDecimal,
//   validatePhone, getPackageCar, removeSelectedPackageCar, savePackageCar, getPackageCarDishHash
// } from '../../../utils/utils'
// import {
//   GOODS_COMMODITY,
//   GOODS_MODEL,
...
//   constructor() {
//     super()
//     const calculateMerchantId = this.$router.params.id
//     const car = getPackageCar()

//     let merchantInfo = this.$router.params.merchantInfo && decodeURIObj(this.$router.params.merchantInfo) || {} // 门店信息
//     const goodsDetail = this.$router.params.goodsDetail && decodeURIObj(this.$router.params.goodsDetail) || {}
//     let dishes = [goodsDetail]
//     let buyNums = goodsDetail.shopDish && goodsDetail.shopDish.minOrderCount ? goodsDetail.shopDish.minOrderCount > goodsDetail.shopDish.buyNums ? goodsDetail.shopDish.minOrderCount : (goodsDetail.shopDish.buyNums || 1) : 1 // 外卖到家购买数量

//     if (calculateMerchantId) {
//       merchantInfo = car[calculateMerchantId].merchant
//       dishes = car[calculateMerchantId].dishes || []
//       if (dishes.length > 0) {
//         const newDishes = []
//         dishes.forEach(dish => {
//           if (dish.shopDish.selected) { // 选中的才结算
//             newDishes.push(dish)
//           }
//         })
//         dishes = newDishes
...
//       buyNums = dishes.reduce((a, dish) => a + dish.shopDish.buyNums, 0)
//     }
...
//     const goodsDetail = dishes[0] // 套餐类型一致， 可以取第一个
//     const { dispatch } = this.props
//     let tabs = []
//     // console.log(goodsDetail);
//     // showToast("当前定位超出外卖配送范围");
//     // 如果传递了 套餐类型 则使用传递的套餐类型（即传递类型，就不允许选择）
//     const productType = this.$router.params.typeValue || goodsDetail.shopDish.productType
//     if (objNotNull(goodsDetail) && objNotNull(goodsDetail.shopDish) && productType) {
//       tabs = GOODS_MODEL.filter(o => productTypeAnd(productType, o.value))
//       this.setState({
//         tabs: tabs.length > 0 ? tabs : [], // 根据商品类型选择下单方式
//         currentTab: tabs.length > 0 ? tabs[0] : {}// 当前选中的下单类型tab
//       })
...
//     const { merchantInfo, dishes } = this.state
//     const { dispatch } = this.props
//     const goodsDetail = dishes[0] // 套餐类型一致， 可以取第一个
...
//           }
//         }
//         if (o.demandPrice <= this.getTotalPrice(true).amount && useCondition) {
...
//           this.state.dishes[0].dishMerchantShippingInfo.map(ele => {
...
//           if ((this.state.dishes[0].dishMerchantShippingInfo.length <= 1 && !curMerchantRange) || locationArea(curMerchantRange, {
//             latitude: location[1],
...
//     navToPage(`/pages/userAddress/userAddress?formPage=orderConfirm&rangeArea=${JSON.stringify(curMerchantRange)}&orderType=${this.state.currentTab.value}`)
//   }
...
//     const { dishes, merchantInfo } = this.state
//     const newDishes = dishes.map(dish => {
//       if (dish.dishId == id || dish.hash == id) {
//         if (dish.shopDish.buyNums <= dish.shopDish.minOrderCount) {
//           showToast(`至少需要购买${dish.shopDish.minOrderCount}个`)
//           return dish
//         }

//         if (dish.shopDish.buyNums === 1) {
//           showToast('至少需要购买一个')
//           return dish
//         }
//         dish.shopDish.buyNums--
//       }

//       // 同步数据到购物车缓存
//       savePackageCar(merchantInfo, dish, dish.shopDish.buyNums)
...
//   // 增加数量
//   addGoods = (shopDishSkus, id) => {
//     const { dishes, merchantInfo } = this.state

//     const newDishes = dishes.map(dish => {
//       if (dish.dishId == id || dish.hash == id) {
//         const max = dish.shopDish.limitBuyNum || 0
//         if (dish.shopDish.buyNums >= shopDishSkus.stock) {
//           showToast('已超过最大库存,不能在添加了')
//           return dish
//         }
//         if (max && dish.shopDish.buyNums >= max) {
//           showToast(`每人限购${max}份`)
//           return dish
//         }
...
//     if (isPrize) return 0
//     let result = { amount: 0, shippingFee: 0, memberPrice: 0, boxPrice: 0, discountFee: 0 }
//     dishes.forEach(dish => {
//       const shopDish = dish.shopDish || {}
...
//       let totalPrice = shopDishSkus.price * shopDish.buyNums

//       // 会员价
//       if (shopDishSkus.memberPrice !== null && shopDishSkus.memberPrice !== undefined && !hasLegendsCard) {
//         totalPrice = shopDishSkus.memberPrice * shopDish.buyNums
//         result.memberPrice += totalPrice;
//       }
...
//           const boxPrice = shopDishSkus.boxPrice * shopDish.buyNums
//           totalPrice += boxPrice
//           result.boxPrice += boxPrice
//         }
//         if (merchantInfo.shippingPrice > 0) { // 配送费
//           totalPrice += merchantInfo.shippingPrice
//           result.shippingFee += merchantInfo.shippingPrice
//         }
//       }
//     })
...
//       deliverFee += shopDish.deliverFee;
...
//             details: a.details || ''
//           })) : [],
//           thirdPartyType: isPrize ? dish.thirdPartyType : null,
...
//     }
//     const baseOrder = {
//       amount: formatCurrency(this.getTotalPrice().amount) || 0, // 以后端计算为准
//       // "discountFee": '',
//       merchantId: merchantInfo.id,
...
//       deliverFee: deliverFee ? formatCurrency(deliverFee) : null,
...
//       thirdPartyType: dishes[0].thirdPartyType,
...
//     if (isBuyCard) {
//       baseOrder.amount = toDecimal(this.getTotalPrice().amount - limitPrice)
...
//           // 清除购物车
//           const { dishes } = this.state
//           const hashs = dishes.map(dish => getPackageCarDishHash(dish))
...
//     if (this.getTotalPrice().amount > balance) return
//     this.setState({
//       payType: 0
//     })
//   }
...
//     // const {dishes = []} = calculateMerchantId ? car[calculateMerchantId] : {}
//     // dishes.length <= 0 && dishes.push(goodsDetail)
//     // let shopDish = dishes.length > 0 ? dishes[0].shopDish : goodsDetail.shopDish || {}
//     // const shopDishSkus = shopDish.currentSku ? shopDish.currentSku : shopDish.shopDishSkus && shopDish.shopDishSkus.length > 0 && shopDish.shopDishSkus[0] || {}
//     const { ajaxLoading = {} } = this.props
//     console.log('dishes', dishes)
//     /* if (!objNotNull(shopDish) && !calculateMerchantId) {
//       return <View />
//     } */
...
//     const priceInfo = this.getTotalPrice()
...
//                                 {
//                                   (shopDish.buyNums > 1 && shopDish.minOrderCount < shopDish.buyNums)
//                                   && (
//                                     <View
//                                       className="btn cut"
//                                       hoverClass="hover"
//                                       hoverStartTime={10}
//                                       hoverStayTime={100}
//                                       onClick={this.subGoods.bind(this, dish.hash || shopDish.id)}
//                                     >
//                                       -
//                                     </View>
//                                   )
//                                 }
//                                 <View className="part">{shopDish.buyNums}</View>
//                                 {
//                                   !(this.formPartnerOrder())
//                                   && (
//                                     <View
...
//                                       hoverStartTime={10}
//                                       hoverStayTime={100}
//                                       onClick={this.addGoods.bind(this, shopDishSkus, dish.hash || shopDish.id)}
//                                     >
//                                       +
//                                     </View>
//                                   )
...
//                           >
//                             ￥
//                             {!priceInfo.boxPrice ? formatCurrency(priceInfo.boxPrice) : '0'}
//                           </Text>
//                         </View>
//                         <View className="flex-row flex-sb flex-ac item">
...
//                           >
//                             ￥
//                             {priceInfo.shippingFee ? formatCurrency(priceInfo.shippingFee) : '0'}
//                           </Text>
//                         </View>
//                       </Block>
//                     )
...
//                     {formatCurrency(isBuyCard ? priceInfo.amount - limitPrice : priceInfo.amount)}
//                   </Text>
//                 </View>
//                 {
...
//                           本单省
//                           <Text>{toDecimal(priceInfo.amount * buyEarn)}</Text>
//                           元
//                         </View>
//                       </View>
//                     </View>
