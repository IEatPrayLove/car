...
import {
  ...
  saveCurrentLocation, showToast, showLoading, hideLoading
} from '../../../utils/utils'
...
export default class MerchantList extends PureComponent {
  ...
  constructor() {
    super()
    const { merchantType } = this.$router.params
    this.state = {
      ...
      pagination: {
        curPage: 0,
        merchantType: merchantType - 0 || ALL,
        sort: 2,
        merchantList: []
      },
      noMore: false,
  ...
  componentDidShow() {
    if (!needLogin()) return
    const merchantType = this.$router.params.merchantType;
    if (merchantType) {
      this.setState({
        pagination: {
          curPage: 0,
  ...
  onShareAppMessage() {
    ...
    return {
      ...
      path: `/package/multiStore/merchantList/merchantList?merchantType=${merchantType}&code=${code || ''}`
    }
  }
