...
class OrderConfirm extends Component {
  ...
  calculateCarMoney = () => {
    ...
    let shippingMoney = this.state.sendSet.shippingType === 2 ? this.state.sendPrice : shippingInfo.shippingPrice || 0
    ...
    if (carList.length > 0) {
      ...
      totalNums = carList.map(o => o.num)
        .reduce((o1, o2) => (o1 + o2)) // 总分数计算

      if (judge) {
        totalPrice = carList.map(o => (o.num * (o.sku.memberPrice ? o.sku.memberPrice : o.sku.price)))
          .reduce((o1, o2) => (o1 + o2)) // 总价格计算(可能会比较复杂)
      } else {
        totalPrice = carList.map(o => (o.num * o.sku.price))
          .reduce((o1, o2) => (o1 + o2)) // 总价格计算(可能会比较复杂)
      }

      totalPackFee = carList.map(o => (!o.sku.freeBoxPrice ? (o.num * o.sku.boxNum * o.sku.boxPrice) : 0))
  ...
  getUserDefaultAddress = () => {
    console.log(this.state.userAddress, '7777')
  ...
  makeOrder = storedPayParams => {
    ...
    const baseOrder = {
      ...
      sendPrice: sendSet.shippingType === 2 ? sendPrice : shippingInfo.shippingPrice || 0,
          ...
          <View className="header">
            {
              currentTab.key === KEY_TASK_OUT
                ? (
                  <Fragment>
                    {
                      (sendSet.shippingType === 2 ? isInArea : locationArea(shippingInfo.shippingRange, {
                        longitude,
                        latitude
                  ...
                  <View className="flex-row flex-sb flex-ac item">
                    ...
                    <View>
                      ...
                      <Text
                        className="price"
                      >
                        {sendSet.shippingType === 2 ? this.state.sendPrice : shippingInfo.shippingPrice || 0}
                      </Text>
                    </View>
                  </View>
                )
