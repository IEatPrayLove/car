...
export default class scanOrderDetail extends Component {
    ...
    componentWillMount() {
        ...
        const preId = Taro.getStorageSync("preId");
        this.setState({
            preId
        })
    }
    ...
    getOrder = () => {
        ...
        console.log(orderSn, openId, "7777");
        if (enterpriseGuid) {
            this.props.dispatch({
    ...
    render() {
        const { orderSn, orderInfo, productList = [], tableFee, enterpriseGuid, wxtoken, openId, memberInfoGuid, brandId, merchantId, areaInfo, peddingProductList, cancelProductList, payedProductList, orderState, preId } = this.state
            ...
            <View className="detailBox">
                ...
                <View className="whiteBox orderDetail">
                    ...
                    <View className="detailItem">
                        ...
                        <Text className="detailWord">{orderInfo.addTime}</Text>
                    </View>
                </View>
                                        ...
                                        if (fromPage === 'index') {
                                            ...
                                            Taro.getStorage({
                                                ...
                                                success: res => {
                                                    ...
                                                    Taro.redirectTo({ url: `/package/otherScanOrder/scanningIndex/scanningIndex?personNum=${perNum}&tableInfo=${tableInfo}&merchantId=${merchantId}&tableId=${tableId}&brandId=${brandId}&payType=${payType}&tableName=${tableName}&newTable=${false}&enterpriseGuid=${enterpriseGuid}&openId=${openId}&orderSn=${orderSn}&wxtoken=${wxtoken}&phone=${phone}&preId=${preId}` })
                                                }
                                            })
                                        } else {
                                            if (enterpriseGuid) {
