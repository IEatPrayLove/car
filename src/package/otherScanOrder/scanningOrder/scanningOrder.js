...
export default class ScanningOrder extends Component {
  ...
  translateData = (tdata) => {
    ...
    const newOrderDetail = {
      ...
      operateTime: tdata.checkoutTime,
      orderActivityList: [],
      orderCount: null,
      orderNum: 0,
  ...
  render() {
    const {
      orderDetail: {
        ...
        peopleNum, mealNumber, couponFee, discountFee, tableFee, settlementItemDTOs
      }, noOrder, enterpriseGuid
    } = this.state
    console.log(this.state.orderDetail, "orderDetail")
    ...
    return (
      <View className="orderBox">
        ...
        <View className="orderInfo">
          ...
          {
            settlementItemDTOs && settlementItemDTOs.length > 0 && (
              settlementItemDTOs.map((item, index) => {
                const { itemName, ioType, moneyAmount, type } = item
                const addOrReduce = ioType == 0 ? "-" : "+";
                return (
                  <View className="orderInfoItem" key={index + type}>
                    <View className="orderFavourable">
                      <Text className="coupon">{itemName}</Text>
                    </View>
                    <View className="orderReduce">
