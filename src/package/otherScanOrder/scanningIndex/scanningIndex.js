...
class ScanningIndex extends Component {
  ...
  componentWillMount() {
    ...
    this.setState({
      ...
      // console.log(this.$router.params.tableInfo,'title')
      Taro.setNavigationBarTitle({
        // title: JSON.parse(this.$router.params.tableInfo).brandName
        title: JSON.parse(this.$router.params.tableInfo).merchantName
      })
    })
    if (getAuthenticate()) {
  ...
  //商品广告弹窗，可加入购物车
  showGoodsAdModal = (goodsAd_id)=>{
    
    if(goodsAd_id)this.setState({ goodsAd_id })
    let goodsArray = [], goods_id = goodsAd_id || this.state.goodsAd_id;
    console.log("静茹了===============>", goods_id)
    if(!goods_id)return;
    this.state.dishList.map(item=>{
      if(item.shopDishProductCats)goodsArray.push(...item.shopDishProductCats);
    });
    let goodsInfo = goodsArray.find(g=>g.id === goods_id);
    console.log("数据啊==================》", this.state.dishList, goodsInfo, goodsArray,goods_id)
    if(goodsInfo){
      this.showDetail(goodsInfo);
    }
  }
  ...
  getAdModal = () => {
    ...
    Taro.setStorageSync("preId", preId);
    dispatch({
      type: 'common/popWindowAction',
      payload: {
        merchantId: preId
      },
      callback: ({ ok, data }) => {
        // 6638328702155358209
        // 测试数据
        // data.dishDTO={thirdNo: '6638328702155358209'};
        // data.adStatus = 1;
        // data.showStatus = 2
        ...
        if (ok) {
          ...
          if (objNotNull(BannerDTO)) {
            ...
            switch (showUserType) {
              case 0: {                       //全部用户
                this.decidePop(userDetail, BannerDTO, showStatus, dishDTO);
                break;
              }
              case 1: {                   //我们这边的会员
  ...
  //判断当次是否进行弹窗
  decidePop = (userDetail, BannerDTO, showStatus, dishDTO ) => {
    const { platformSystemBannerPopSettingVM: [{ cycleType, popNumber }] } = BannerDTO;
    if (userDetail) {
      const lastModal = this.getModalCount();
      if (cycleType == 'FIRST') {
        if (lastModal) {
          if(showStatus === 2) return this.showGoodsAdModal('');
          this.setState({
            indexModal: {}
          })
        } else {
          this.setModalCount({ 'time': new Date(), 'count': 1 }) 
          if(showStatus === 2) return this.showGoodsAdModal(dishDTO.thirdNo);
          this.setState({
  ...
  calculateBuyNum = ({ shoppingCar, catItem, clear }) => {
    ...
    return catItem && catItem.map(goods => {
  ...
  buyNow = noLink => {
    ...
    if (enterpriseGuid) {
      //提交购物车
      dispatch({
        type: 'common/otherSubmitCartAction',
        payload: {
          headerMessage: {
            enterpriseGuid,
            wxtoken,
  ...
  ismember = (callback) => {
    ...
    dispatch({
      type: 'common/otherMemberMesAction',
      payload: {
        headerMessage: {
          openId,
          wxtoken,
  ...
  getOtherPlatformData = wxPermission => {
    ...
    const { dispatch } = this.props;
    dispatch({
      type: 'common/getOtherPlatformGoodsAction',
      payload: {
        headerMessage: {
          enterpriseGuid,
          wxtoken,
  ...
  getOtherPlatformCart = (callback) => {
    ...
    const { enterpriseGuid, openId, wxtoken } = this.state
    dispatch({
      type: 'common/getOtherPlatformCartcontent',
      payload: {
        headerMessage: {
          enterpriseGuid,
          openId,
  ...
  otherCartDataPlay = (callback) => {
    ...
    console.log(itemInfoDTO, "itemInfoDTO")
    dispatch({
      type: 'common/otherPlatformJoinCart',
      payload: {
        headerMessage: {
          enterpriseGuid,
          openId,
  ...
  otherClearCart = (callback) => {
    ...
    console.log("清空购物车")
    this.props.dispatch({
      type: 'common/clearCartContentAction',
      payload: {
  ...
  getNowOrder = (callback) => {
    ...
    if (enterpriseGuid) {
      this.props.dispatch({
        type: 'common/getOtherPlatOrdersAction',
        payload: {
          headerMessage: {
            enterpriseGuid,
            openId,
  ...
  render() {
    const {
      ...
      dishInfo, indexModal, statusLoading
    } = this.state
    // console.log(Taro.getStorageSync("merchantId"))
    ...
    * */
    ...
    return (
      <View className="scanningBox">
        ...
        {/* {
          (effects['common/getOtherPlatformCartcontent'] || !socketIsSuccess) && (
        ...
        {/* {
          (effects['common/otherPlatformJoinCart'] || !socketIsSuccess) && (
            <View className="loading1">
              <Image className='showImg' src={`${STATIC_IMG_URL}/loading.gif`} />
            </View>
          )
        ...
        {
          (effects['common/otherSubmitCartAction'] || !socketIsSuccess) && (
            <View className="loading1">
              <Image className='showImg' src={`${STATIC_IMG_URL}/loading.gif`} />
            </View>
          )
              ...
              {!enterpriseGuid
                ...
                : (payType === 1 && (
                  ...
                  >
                    ...
                    />
                    <Text style={{ fontSize: '24rpx' }}>去结算</Text>
                    {
        ...
        {/* 商品详情弹窗 */}
        {
          ...
          && (
            ...
            >
              <View className="dishDetailBox" onClick={(e)=>{e.stopPropagation()}}>
                <Image
                  src={`${STATIC_IMG_URL}/icon/icon_shutDown.png`}
                  className="closeImg"
                  onClick={this.closeDetail}
                ...
                <View className="dishInfoBox">
                  ...
                  <View className="dishInfoFooter">
                    ...
                    {!!this.state.goodsAd_id && <View>
                      {
                        // const validAttrs = shopDishAttributes.length > 0 && shopDishAttributes.filter(a => a.details.length > 1)
                        !(dishInfo.shopDishSkus && dishInfo.shopDishSkus.length > 0 && (Math.max(...dishInfo.shopDishSkus.map(a => Number(a.stock))) > 0))
                          ? <View className="sale-out">已售罄</View>
                           : (dishInfo.shopDishSkus && dishInfo.shopDishSkus.length > 1 || (dishInfo.shopDishAttributes && dishInfo.shopDishAttributes.filter(a => a.details.length > 1).length>0 ))
