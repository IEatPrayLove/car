...
export default class scanningConfirm extends Component {
      ...
      callback: ({ ok, data }) => {
        if (ok) {
          if (data.tdata) {
            ...
            const unUsableRedPackage = [];
            // 使用门槛-满多少金额起用 thresholdAmount 门槛金额
            const thresholdAmount = memberVolumeList.useThresholdFull;
            // if (redPackage.length > 0) {
            //   redPackage.map(o => {
            //     let useCondition = false
            //     switch (o.couponType) {
  ...
  calculateTotalAmount = () => {
    ...
    if (enterpriseGuid) {
      console.log(2)
      return payAmount != null ? payAmount : productFee
    } else {
      console.log(3)
      const {
  ...
  useVolume = (volume, index) => {
    ...
    console.log(this.state.checkVolume, "选择")
    if (i === index) {
      this.setState({
        checkVolume: {},
        i: null
      ...
      // if (volume.useThresholdFull > this.calculateTotalAmount()) {
              ...
              <View className="flex-col package-wrap">
                <View className="flex-row flex-ac flex-sb modal-header">
                  <Button
                    ...
                    hoverClass="hover"
                    onClick={() => {
                      console.log(this.state.checkVolume, "取消")
                      this.setState({
                        showRedPackModal: false,
                        // checkVolume: {},
                      });
                  ...
                  {usableRedPackage.map((o, i) => {
                    ...
                    return (
                      <View
                        ...
                        hoverStayTime={100}
                        key={i}
                        onClick={this.useVolume.bind(this, o, i)}
                      >
                        <View className="flex-col flex-ac flex-jc left">
                          <View>
