import Taro, { useEffect, useState, showLoading, hideLoading } from '@tarojs/taro'
import {
...
export default function Payment({
  ...
  const translateCardList = (cardlist, memberInfo) => {
    const data = {
  ...
  useEffect(() => {
    ...
    if (otherstate) {
      ...
      dispatch({
        ...
        callback: ({ ok, data }) => {
          if (ok && data.code == 0) {
            // const { cardName,memberBalance,originAmount,payAmount,supportMember,supportPreference} =data.tdata
            // const card = {
            //   cardName,
            //   cardMoney:memberBalance,
            // }
            ...
            dispatch({    //请求会员数据
              ...
              callback: ({ ok, data }) => {
                ...
                if (ok && data.code == 0) {
                  ...
                  const newdata = translateCardList(selectedCard, memberInfo, unpayList)//   若需要不显示余额不足的会员卡，将这里的 selectedCard 改为list 即可;   
