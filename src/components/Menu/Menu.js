...
export default class Menu extends Component {
  ...
  onOrderNow = () => {
    ...
    this.setState({ showCarDetail: false })
    // saveTempBuyCar(CAR_TYPE_SHOP, this.props.merchant.id, {});
    saveBuyCar({
      dishes: this.state.currentCar,
      merchant: this.props.currentMerchant,
      shippingInfo: this.props.shippingInfo
    })
      .then(() => {
        navToPage('/package/multiStore/takeOutConfirm/takeOutConfirm')
      })
  }
                ...
                <View className="carContainer">
                  ...
                  </View>
                  {
                    (currentMerchant.platFormMerchantDTO && (currentMerchant.platFormMerchantDTO.outerOrderMod & SHOP_MODE_ENUM.SHIPPING.key)) === SHOP_MODE_ENUM.SHIPPING.key ? shippingInfo && shippingInfo.startPrice ? (
                      ...
                      >
                        {
                          shippingInfo && shippingInfo.startPrice && (toTalPrice < shippingInfo.startPrice)
                            ? `￥${shippingInfo.startPrice}起送`
                            : '立即下单'
                        }
                      </Button>
                    ) : (
                        ...
                        >
                          {
                            shippingInfo && shippingInfo.sendingPrice && (toTalPrice < shippingInfo.sendingPrice)
                              ? `￥${shippingInfo.sendingPrice}起送`
                              : '立即下单'
                          }
                        </Button>
                      )
