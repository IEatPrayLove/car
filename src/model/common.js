...
export default {
  ...
  state: {
    ...
    tabBarConfig: [],
    isFirstIntoApp: false, // 是否是启动app而不是其它页面跳转的
    isFullScreen: false,
  },
  ...
  effects: {
    ...
    *getAllMerchantInfosAction({ payload, callback }, { call, put }) {
      const res = yield call(getAllMerchantInfos, payload)
      isFunction(callback) && callback(res)
    },
    ...
    * 另一平台加入购物车
     ...
     * @param call
    */
    *otherPlatformJoinCart({ payload, callback }, { call }) {
   ...
   */
    ...
    *getTabBarAction({ payload, callback }, { call, put } ){
      const res = yield call(getTabBar, payload);
      if(res.ok){
        yield put({
          type: 'getTabBarConfigResult',
          payload: res.data,
        })
      }
      isFunction(callback) && callback(res);
  ...
  reducers: {
    ...
    getTabBarConfigResult(state, { payload }) {
      return {
        ...state,
        tabBarConfig: payload,
      }
    },
    setFirstIntoApp(state, {payload}){
      return {
        ...state,
        isFirstIntoApp: payload,
      }
    },
