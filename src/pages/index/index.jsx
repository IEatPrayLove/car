...
@connect(({
  loading: { effects },
  common: { userDynamic, isFirstIntoApp }
}) => ({
  effects,
  userDynamic,
  isFirstIntoApp,
...
class Index extends Taro.PureComponent {
  ...
  componentDidShow() {
    ...
    try {
      ...
      console.log("检测订单异常：", e);
    }
    // 获取套餐购物车数量
    this.existsPackageCar();
  }
  ...
  existsPackageCar = () => {
    ...
    Object.keys(car).forEach(merchantId => {
      // car[merchantId].dishes.forEach(dish => buyNums += dish.shopDish.buyNums)
      buyNums += car[merchantId].dishes.length;   // 只计算商品件数 不计算每件商品的份数
    })

    console.log("buyNums", buyNums);
    this.setState({ packageCartNum: buyNums })

  }
  toIndexPage=()=>{
    // 前往自定义的首页
    console.log("执行")
    if(!this.props.isFirstIntoApp)return;
    this.props.dispatch({
      type: "common/setFirstIntoApp",
      payload: false,
    })
    this.props.dispatch({
      ...
      callback:({ok, data=[]})=>{
        if(ok){
          const tabItem = data.find(n=>n.isHome);
          if(tabItem){
            const currentPath = '/'+Taro.getCurrentPages()[Taro.getCurrentPages().length-1].route;
            const url = getTabBarPages(tabItem);
            console.log("url==========>", url) 
  ...
  initialize = () => {
    ...
    dispatch({
      ...
      callback: res => {
        ...
        console.log(appletsAd, 'appletsAd')
        this.setState({
  ...
  goToLegendsCard = () => {
    ...
    if (judgeLegendsCard(islandUserMemberDTO)) {
      navToPage('/package/legendsUserCenter/legendsUserCenter')
    } else {
          ...
          list.map(ele => {
            ...
            return (
              <View
                ...
                key={id}
                onClick={() => {
                  navToPage(`/package/propagandaRewardDetail/propagandaRewardDetail?id=${id}`)
                }}
              >
  ...
  getAllmerchanList = () => {
    ...
    this.props.dispatch({
      ...
      callback: res => {
        if (res.ok) {
          console.log(res.data, "allMerchantList")
          this.setState({
  ...
  render() {
    ...
    return (
      <View className="pageContainer">
        ...
        {/* {
          effects['common/getPlatFormDetailAction'] && (
            <View className="loading1">
            ...
            <View className="member" style={{ marginTop: '-10px' }}>
              <View className="title">
                ...
                <View
                  ...
                  onClick={() => {
                    navToPage('/package/propagandaReward/propagandaReward')
                  }}
                >
                  <Text>更多</Text>
            ...
            <View className="member" style={{ marginTop: '-10px' }}>
              <View className="title">
                ...
                <View
                  className="more flex-row flex-ac"
                  onClick={() => {
                    Taro.reLaunch({ url: '/pages/specialOffer/specialOffer' })
                  }}
                >
                  <Text>更多</Text>
                  <IconFont value="icon-arrow-right-copy-copy" size={30} />
        ...
        >
          ...
          {!platformDetail.homeModel ? (
            <View className="member">
              ...
              {/* <AtLoadMore status={merchantNoData ? 'noMore' : 'loading'} /> */}
            </View>
          )
            : (
              <View className="member">
        ...
        {orderState &&
          <View className="orderTag">
            <ToOrderDetail merchantDetail={null} orderSn={orderSn ? orderSn : null} wxtoken={wxtoken ? wxtoken : null} enterpriseGuid={enterpriseGuid ? enterpriseGuid : null} openId={openId ? openId : null} payType={1} memberInfoGuid={memberInfoGuid ? memberInfoGuid : null} merchantId={merchantId ? merchantId : null} brandId={brandId ? brandId : null} currentMerchant={currentMerchant ? currentMerchant : null} tableInfo={tableInfo ? tableInfo : null}></ToOrderDetail>
          </View>}
        <Footer />
        ...
        {/* 套餐购物车 */}
        {packageCartNum && <View className="packageCart">
          <View className="packageCartContent" onClick={() => {
            goPackageCar()
          }}>
            <Image className="packageCartImg" src={STATIC_IMG_URL + '/icon/packageCart.png'}></Image>
            <View className="num">{packageCartNum}</View>
          </View>
        </View>}
      {/* 自定义导航 */}
      <custom-tab-bar>
        {/* 关注公众号，因为自定义导航导致的bug所以放在了tabBar里  */}
        {accountModal && (
          <View className="attention">
            <OfficialAccount
              onError={(e)=>{console.log("OfficialAccount onError", e)}}
              onLoad={(e) => {
                console.log("OfficialAccount onLoad", e)
