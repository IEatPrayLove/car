...
import {
  ...
  setShareInfo, parseQuery, getShareInfo, findMinSku, savePackageCar, goPackageCar, getPackageCar
} from '../../utils/utils'
...
class GoodsDetail extends Component {
  ...
  constructor() {
    ...
    this.state = {
      ...
      packageCartNum: 0, // 购物车数量
      buyMode: '', // buy 立即购买 car 加入购物车
    }
  ...
  existsPackageCar = () => {
    ...
    Object.keys(car).forEach(merchantId => {
      ...
      buyNums += car[merchantId].dishes.length;
    })

    // if (buyNums > 0) {
    this.setState({
      packageCartNum: buyNums,
    })
    // }
  }
  ...
  // 去购物车
  goPackageCartPage = () => {
    const { packageCartNum } = this.state;
    if (packageCartNum > 0) {
      goPackageCar()
    } else {
      Taro.showToast({
  ...
  // 添加购物车
  addCart = (merchant, dish) => {
    // this.setState({tc_island_package_cart: true});
    // key 为 tc_island_package_cart
    savePackageCar(merchant, dish, null, (ok) => {
      if (ok) {
        Taro.showToast({
          title: '添加成功',
          icon: 'success',
  ...
  buyNow = (goodsDetail, mode) => {
    ...
    this.props.dispatch({
      ...
      callback: ({ ok, data }) => {
        ...
        const { buyMode } = this.state

        if ((buyMode || mode) === 'car') {
          this.addCart(this.props.merchantInfo, { ...goodsDetail, content: '' });
          return;
        }
        navToPage(`/package/multiStore/packageOrderConfirm/packageOrderConfirm?merchantInfo=${encodeURIObj(this.props.merchantInfo)}&goodsDetail=${encodeURIObj({ ...goodsDetail, content: '' })}`)
      }
    })
  }
  ...
  render() {
    const {
      ...
      userNotice, notMember, userLocation, packageCartNum
    } = this.state;
    console.log(currentSku, 'currentSku', buyNums, 'buyNums')
    const {
            ...
            <View className="flex-row flex-sb goods-detail-footer">
              ...
              <View className="flex-row flex-jc flex-ac footerInfo">
                <Button
                  ...
                  open-type="contact"
                >
                  <Image src={`${STATIC_IMG_URL}/package-service.png`} />
                  <View className="footerBtnTxt">联系客服</View>
                </Button>
                <View className="footerLine" />
                <Button
                  className="btn go-home go-cart"
                  hoverClass="hover"
                  onClick={this.goPackageCartPage}
                >
                  <View className="icon_box">
                    <Image src={`${STATIC_IMG_URL}/package-cart.png`} />
                    {!!packageCartNum && <View className="tags">{packageCartNum}</View>}
                  </View>
                  <View className="footerBtnTxt">购物车</View>
                </Button>
              </View>
              <View className="footer-btns">
                <Button className="add-cart" onClick={() => {
                  if (shopDishSkus.length > 1 || shopDishAttributes.length > 0) { // 多规格,多属性商品
                    ...
                    if (shopDish.minOrderCount) {
                      let butNums = shopDish.minOrderCount;
                      this.setState({
