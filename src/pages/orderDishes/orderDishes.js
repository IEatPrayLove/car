...
@connect(({ orderDishes, loading: { effects } }) => ({
  // ajaxLoading: loading,
...
class OrderDishes extends Taro.Component {
      ...
      callback: ({ ok, data }) => {
        if (ok) {
          ...
          if(!data || data.length <= 0){
            showToast('暂无门店支持外卖，请选择其他服务吧～')
            return
          }

          this.setState({
            merchantId: data && data.length > 0 ? data[0].id : 0,
            flag: true,
          }, () => {
            this.initData()
          })
  ...
  render() {
    ...
    return (
      <View>
        {
          (effects['orderDishes/getPlatFormSystemSettingByIdAction'] || !flag) && (
