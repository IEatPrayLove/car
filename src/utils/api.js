...
export async function isPartner(params) {
  ...
  return $get(`${config.API_MERCHANT}/is-it-a-partner?platformId=${params.platformId}`)
}
...
export async function getStarSelectList(params) {
  return $get(`${config.API_PLATFORM}/platform-star-dish/page?type=${params.type}&platformId=${params.platformId}&lng=${params.lng}&lat=${params.lat}&dishName=${params.dishName}&categoryId=${params.categoryId}&page=${params.page}&size=${params.size}&sort=topTime,desc`)
}
...
// 获取后台分销商品
export function newGetDistributorProduct(params) {
  return $get(`${config.API_PLATFORM}/platform-distributor/dish-info-show-tcd-merchant?platformId=${config.PLATFORM_ID}&page=${params.page}&size=${params.size}&checkState=${params.checkState}`)
}
// 屏蔽
...
export async function getAllMerchant(params) {
  return $get(`${config.API_MERCHANT}/list-merchant-info-with-platform?page=${params.page}&size=${params.size}&platformId=${params.platformId}&brandId=${params.brandId || ''}&outerOrderMod=${params.outerOrderMod}&position=${params.position}&orderSort=2&orderType=${params.orderType}`)
}
// 搜索门店
...
// 获取又有门店信息
export async function getAllMerchantInfos(params) {
  return $get(`${config.API_MERCHANT}/get-tcd-merchant-business-info/condition?state=1&tcdPlatformId=${PLATFORM_ID}&brandId=&search=&page=0&size=99999&sort=id,desc`)
}
// 获取所有门店数量
...
export async function getTabBar() {
  return $get(`${config.API_PLATFORM}/platform-bottom-navigations?platformId=${config.PLATFORM_ID}`)
}
