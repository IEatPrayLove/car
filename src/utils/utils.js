...
export function loginCompleteGo(delta = 1) {
  ...
  switch (readLoginCompleteGo()) {
    case 'distributor':
      saveLoginCompleteGo('')
      // Taro.reLaunch({url: "/pages/distributor/distributorLayout"});
...
export function advertisingLinks(params, _this) {
  ...
  switch (linkType) {
    ...
    case 3: {
      ...
      if (functionPosition === 6) { // 合伙人
        navToPage(NAV_LINK[functionPosition])
        return
      }
      if (functionPosition === 7) { // 储值
        navToPage(NAV_LINK[functionPosition])
        return
      }
      if (functionPosition === 1) { // 特惠商品
    ...
    case 6: {
      const {
        id = dishId, brandName, brandLogo, brandDetailPic
      } = brandDTO
      _this.$preload({
 ...
 * 获取套餐购物车
 */
export function getPackageCar() {
  const car = Taro.getStorageSync(getCacheName('package_cart'))
  return car || {}
}
 ...
 * 存储套餐购物车
 */
export function savePackageCar(merchant, dish, nums, callback = () => { }) {
  const car = getPackageCar() || {}
  const { dishes = [] } = car[merchant.id] || {}
  const dishHash = getPackageCarDishHash(dish)
  const carDishes = dishes.filter(item => item.hash === dishHash)

  if (carDishes.length > 0) {
    const dishInCar = carDishes[0]
    const max = dishInCar.shopDish.limitBuyNum || ''
    if (dish.buyNums >= dishInCar.shopDish.stock) {
    ...
    if (dish.length > 99){
      ...
      // 计算限购数量 包含规格属性
      const totalBuyNum = dishes.filter(item => item.hash == dishHash).reduce((totalBuyNum, item) => totalBuyNum += item.shopDish.buyNums, 0)

    // if (max && totalBuyNum >= max) {
    //   showToast(`每人限购${dish.shopDish.limitBuyNum}份`)
    //   return
    // }

    dishInCar.shopDish.buyNums = nums || dishInCar.shopDish.buyNums + 1
    if (max && dishInCar.shopDish.buyNums > max) {
      showToast(`每人限购${dish.shopDish.limitBuyNum}份`)
      dishInCar.shopDish.buyNums -= 1;
    }
    dishInCar.shopDish.selected = dish.shopDish.selected
  } else {
  ...
  const originPrice = dishes.reduce((a, dish) => {
    const shopDishSkus = dish.currentSku ? dish.currentSku : dish.shopDish.shopDishSkus && dish.shopDish.shopDishSkus.length > 0 && dish.shopDish.shopDishSkus[0] || {}
    return a + (shopDishSkus.originalPrice || shopDishSkus.price) * dish.shopDish.buyNums
  }, 0)
  const amount = dishes.reduce((a, dish) => {
    const shopDishSkus = dish.currentSku ? dish.currentSku : dish.shopDish.shopDishSkus && dish.shopDish.shopDishSkus.length > 0 && dish.shopDish.shopDishSkus[0] || {}
    return a + (shopDishSkus.memberPrice || shopDishSkus.price) * dish.shopDish.buyNums
  }, 0)

  console.log(originPrice, amount)

  // 购物车结构
  car[merchant.id] = {
    merchant: { ...merchant, name: merchant.merchant_name },
    dishes,
    discountVal: originPrice - amount,
    amount
  }

  try {
    Taro.setStorageSync(getCacheName('package_cart'), car);
    if (typeof callback === 'function') callback(true);
  } catch (e) {
    if (typeof callback === 'function') callback(false);
    console.log("套餐购物车添加商品失败===>", e);
  }
}
 ...
 * 清空套餐购物车
 */
export function clearPackageCar(merchantId) {
  const car = getPackageCar() || {}
  delete car[merchantId]
 ...
 * 移除过期套餐
 */
export function removeAnyPackageCar() {
  const car = getPackageCar() || {}
  Object.keys(car).forEach(id => {
    car[id].dishes.forEach((dish, i) => {
      if (dish.dishState != 2) {
        car[id].dishes.splice(i, 1)
      }
    })
    if (car[id].dishes.length <= 0) {
      delete car[id];
    }
  })
 ...
 * 移除选中套餐
 */
export function removeSelectedPackageCar(hashs) {
  const car = getPackageCar() || {}
  Object.keys(car).forEach(id => {
    let merchantDishes = []
    car[id].dishes.forEach((dish, i) => {
      if (hashs.indexOf(getPackageCarDishHash(dish)) == -1) {
        merchantDishes.push(dish)
      }
    })
 ...
 * 跳转到套餐购物车
 */
export function goPackageCar() {
  Taro.navigateTo({ url: '/packageSecond/packageCart/packageCart' })
}
 ...
 * @returns {*}
 */
export function getPackageCarDishHash(dish) {
  let str = dish.id
  str += dish.shopDish.selectedSkuAndAttrStr ? 'sku_' + dish.shopDish.selectedSkuAndAttrStr : ''
  return md5(str)
}
/*
 * 获取tabBar对应的页面path
 * @param {*} item 
 */
export function getTabBarPages(item){
  if(!item)return '';
  const pathKeys = {
    "HOME_PAGE": "/pages/index/index", // 首页
    "TAKE_OUT_FOOD": "/pages/orderDishes/orderDishes", // 外卖点餐页
    "VAULT_PAGE": "/pages/treasury/index", // 金库
    "PARTNER_CENTER": "/pages/distributorIndex/distributorIndex", //合伙人中心
    "PERSONAL_CENTER": "/pages/mine/mine", // 个人中心
    "BONUS_POOLS": "/pages/legendsUserCenter/legendsUserCenter", // 奖金池
    "GOODS_ON_SHELVES": "/pages/specialOffer/specialOffer", // 货架商品频道页
    "ALL_MERCHANT": "/package/multiStore/merchantList/merchantList", // 全部门店列表页
    "ALL_BRAND": "/package/multiStore/brandList/brandList", // 全部品牌店列表页
    "CONSUMER_ORDER": "/pages/order/order", // 消费订单
    "SHOPPING_CART": "/packageSecond/packageCart/packageCart", // 购物车
  }
  if(item.linksType === 'OTHER'){
    return item.linksPage;
  }else if(pathKeys[item.linksType]){
    let url = pathKeys[item.linksType]
    if(item.linksType === 'TAKE_OUT_FOOD' && url){
      url=`${url}?id=${item.brandId}`;
    }
    return url;
  }else{
    console.warn("此tabBar无效===================》", item);
    return '';
  }
}
